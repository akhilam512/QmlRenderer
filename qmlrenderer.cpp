...

{
    ...
    std::unique_ptr<QEvent> test = std::make_unique<QEvent>(QEvent::UpdateRequest);
    qEvent.reset(test.get());
}
...
void QmlRenderer::renderEntireQml()
{
    ...
    if (m_currentFrame < m_frames) {
        ...
        updateRequest.reset(qEvent.get());
        QCoreApplication::postEvent(this, updateRequest.get());
    } else {
        //Finished
        cleanup();
    }
...
void QmlRenderer::renderOneFrame()
{
    while(m_animationDriver->elapsed() < m_frameTime) {
        qDebug() << "ok";
        m_animationDriver->advance();
        updateRequest = std::make_unique<QEvent>(QEvent::UpdateRequest);
        QCoreApplication::postEvent(this, updateRequest.get());
    }
    qDebug() << "ok2";
...
void QmlRenderer::renderSingleFrame()  // render frames till required frame without saving frames
{
    // Render frame
    m_renderControl->polishItems();
    m_renderControl->sync();
